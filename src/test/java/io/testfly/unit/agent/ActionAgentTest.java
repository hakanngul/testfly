...
/**
 * Unit tests for Sprint 4 Goal-Oriented Dynamic Steps (ActionCompiler,
 * ActionExecutor, ActionCache).
 */
...
public class ActionAgentTest {
  ...
  @Test
  public void testSelectActionChoosesOptionByVisibleText() {
    WebElement selectEl = mock(WebElement.class);
    when(selectEl.isDisplayed()).thenReturn(true);
    when(mockDriver.findElement(By.cssSelector("#state"))).thenReturn(selectEl);
    when(mockDriver.findElements(By.cssSelector("#state"))).thenReturn(List.of(selectEl));

    try (MockedConstruction<Select> mocked = mockConstruction(Select.class)) {
      ActionStep step = new ActionStep(ActionType.SELECT, "#state", "California", "Pick state");
      ActionExecutor.executeStep(mockDriver, step, 1, 1, Duration.ofSeconds(1));

      assertEquals(mocked.constructed().size(), 1);
      verify(mocked.constructed().get(0)).selectByVisibleText("California");
    }
  }

  @Test(expectedExceptions = IllegalArgumentException.class)
  public void testSelectActionRequiresValue() {
    WebElement selectEl = mock(WebElement.class);
    when(selectEl.isDisplayed()).thenReturn(true);
    when(selectEl.getTagName()).thenReturn("select");
    when(mockDriver.findElement(By.cssSelector("#state"))).thenReturn(selectEl);
    when(mockDriver.findElements(By.cssSelector("#state"))).thenReturn(List.of(selectEl));

    ActionStep step = new ActionStep(ActionType.SELECT, "#state", null, "Pick state");
    ActionExecutor.executeStep(mockDriver, step, 1, 1, Duration.ofSeconds(1));
  }

  @Test
  public void testNavigateActionWithAbsoluteUrlSkipsLocatorParsing() {
    ActionStep step = new ActionStep(ActionType.NAVIGATE, null,
        "https://shop.example.com/inventory.html", "Go to inventory");

    ActionExecutor.executeStep(mockDriver, step, 1, 1, Duration.ofSeconds(1));

    verify(mockDriver).get("https://shop.example.com/inventory.html");
  }

  @Test
  public void testNavigateActionResolvesRelativePathAgainstBaseUrl() {
    TestFlyConfig.Execution exec = new TestFlyConfig.Execution();
    exec.setBaseUrl("https://shop.example.com/");
    config.setExecution(exec);

    ActionStep step = new ActionStep(ActionType.NAVIGATE, "", "/cart.html", "Go to cart");
    ActionExecutor.executeStep(mockDriver, step, 1, 1, Duration.ofSeconds(1));

    verify(mockDriver).get("https://shop.example.com/cart.html");
  }

  @Test
  public void testResolveTargetUrlVariants() {
    TestFlyConfig.Execution exec = new TestFlyConfig.Execution();
    exec.setBaseUrl("https://shop.example.com/");
    config.setExecution(exec);

    assertEquals(ActionExecutor.resolveTargetUrl("https://other.example.com/x"), "https://other.example.com/x");
    assertEquals(ActionExecutor.resolveTargetUrl("/cart"), "https://shop.example.com/cart");
    assertEquals(ActionExecutor.resolveTargetUrl("cart"), "https://shop.example.com/cart");
    assertEquals(ActionExecutor.resolveTargetUrl("\"/cart\""), "https://shop.example.com/cart");
  }

  @Test(expectedExceptions = IllegalArgumentException.class)
  public void testResolveTargetUrlRejectsRelativePathWithoutBaseUrl() {
    config.setExecution(null);
    ActionExecutor.resolveTargetUrl("/cart");
  }

  @Test(expectedExceptions = IllegalArgumentException.class)
  public void testResolveTargetUrlRejectsEmptyValue() {
    ActionExecutor.resolveTargetUrl("   ");
  }

  @Test
  public void testParsePlanWithSelectAndNavigateSteps() {
    String rawJson = """
        {
          "goal": "Checkout flow",
          "steps": [
            {"action": "SELECT", "locator": "#state", "value": "California", "description": "Pick state"},
            {"action": "NAVIGATE", "locator": "", "value": "/cart.html", "description": "Go to cart"}
          ]
        }
        """;

    ActionPlan plan = ActionCompiler.parsePlan(rawJson, "Checkout flow", "/checkout");

    assertEquals(plan.steps().size(), 2);
    assertEquals(plan.steps().get(0).action(), ActionType.SELECT);
    assertEquals(plan.steps().get(0).locator(), "#state");
    assertEquals(plan.steps().get(0).value(), "California");
    assertEquals(plan.steps().get(1).action(), ActionType.NAVIGATE);
    assertEquals(plan.steps().get(1).value(), "/cart.html");
  }

  @Test
  public void testBuildPromptAdvertisesSelectAndNavigate() {
    String prompt = ActionCompiler.buildPrompt("https://shop.com/cart", "Cart",
        "<select id='state'><option>California</option></select>", "Pick California then go to cart");
