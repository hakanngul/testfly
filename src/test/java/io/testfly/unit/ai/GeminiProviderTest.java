...
public class GeminiProviderTest {
  ...
  public void buildRequestBody_escapesJsonCorrectly() {
    ...
    Assert.assertTrue(body.contains("\"maxOutputTokens\":2048"));
  ...
  public void extractContent_emptyOrInvalid_returnsNull() {
    ...
    Assert
        .assertNull(GeminiProvider.extractContent("{\"candidates\":[{\"content\":{\"parts\":[{\"text\":\"   \"}]}}]}"));
  }

  @Test
  public void call_invalidModelName_rejectedWithoutHttpCall() {
    GeminiProvider provider = new GeminiProvider();
    // Model with invalid characters (e.g. path traversal or query params) must be
    // rejected
