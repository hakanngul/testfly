...
public class MailboxClientTest {
    ...
    private static MailboxClient clientWithEmails(List<Email> emails) throws Exception {
        io.testfly.config.TestFlyConfig.Email cfg =
                new io.testfly.config.TestFlyConfig.Email();
        cfg.setTimeoutSeconds(5);
        cfg.setPollIntervalMs(50);

        Constructor<MailboxClient> ctor =
                MailboxClient.class.getDeclaredConstructor(
                        io.testfly.email.EmailProvider.class,
                        io.testfly.config.TestFlyConfig.Email.class);
        ctor.setAccessible(true);

        io.testfly.email.EmailProvider fakeProvider = new io.testfly.email.EmailProvider() {
