 ...
 *
 * <p>Configured via {@code testfly.yml}:
 * <pre>
 * ci:
...
public final class BuildThresholdEnforcer {
    ...
    public static void enforce(TestFlyConfig config, Collection<TestTiming> timings) {
        TestFlyConfig.Ci ci = config.getCi();
        if (ci == null) {
        ...
        if (threshold > 0) {
            ...
            System.out.printf("[TestFly] Pass rate: %.1f%% (required ≥ %.1f%%) ✓%n",
                    passRate, threshold);
        }
        ...
        if (maxFlaky >= 0) {
            ...
            System.out.printf("[TestFly] Flaky tests: %d (max allowed: %d) ✓%n",
                    flaky, maxFlaky);
        }
    }
}
