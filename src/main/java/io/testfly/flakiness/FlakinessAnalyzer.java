 ...
 *
 * <p>
 * Reads the last {@code flakiness.historyRuns} JSON files from
 ...
 *
 * <p>
 * Results are exported to {@code target/flakiness-report.json} and surfaced in
...
public final class FlakinessAnalyzer {
    ...
    /**
     * Holds the result of the last analysis for this JVM run (used by report
     * generator).
     */
    ...
    public static void analyze() {
        if (!io.testfly.config.FeatureGate.enabled(io.testfly.config.FeatureGate.FLAKINESS, true)) {
            LOG.fine("[FlakinessAnalyzer] Disabled via features.flakiness=false — skipping analysis.");
            return;
        }
        try {
            TestFlyConfig.Flakiness cfg = config();
    ...
    static List<File> loadHistoryFiles(int maxRuns) {
        File dir = io.testfly.reporting.ReportPaths.metricsHistoryDir();
    ...
    private static TestFlyConfig.Flakiness config() {
        try {
            return TestFlyContext.getConfig().getFlakiness();
        } catch (Exception e) {
            return null;
        }
    }
