...
public interface ApiAuth {
    ...
    public static ApiAuth apiKeyQuery(String paramName, String apiKey) {
        return new ApiAuth() {
            ...
            @Override
            public void applyToClient(io.testfly.client.ApiClient client) {
    ...
    /**
     * Simple Digest placeholder — sends Basic-like header with Digest prefix. For
     * full RFC 2617 use a custom interceptor.
     */
        ...
        return builder -> {
            try {
                ...
                String sig = Base64.getEncoder()
                        .encodeToString(mac.doFinal(apiKey.getBytes(java.nio.charset.StandardCharsets.UTF_8)));
     ...
     * Token is fetched on first use and cached per {@code tokenUrl + clientId}
     * until expiry.
     *
     * <pre>
    ...
    static final class OAuth2TokenCache {
        ...
        private static CachedToken fetchPasswordToken(String tokenUrl, String clientId, String clientSecret,
            ...
            try {
                ...
                throw new RuntimeException("[ApiAuth] Failed to fetch OAuth2 password token from: " + tokenUrl, e);
            }
        }
