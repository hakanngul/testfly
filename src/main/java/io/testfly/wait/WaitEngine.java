...
public final class WaitEngine {
     ...
     *
     * <p>
     * Use sparingly — prefer {@link #waitForVisible(By)} and other explicit waits.
     */
     ...
     *
     * <p>
     * Element-based variant used by {@link io.testfly.locator.Locator} after its
     * candidate set has been resolved. Self-healing is applied earlier, at
     * resolution
    ...
    /**
     * Waits until the given already-resolved element is clickable (visible and
     * enabled).
     *
     * <p>
     * Element-based variant used by {@link io.testfly.locator.Locator} after its
     ...
     * Use {@link #waitForAttributeContains(By, String, String)} for a substring
     * match.
     *
     * <pre>
    ...
    /**
     * Waits until at least {@code minimumCount} elements matching the locator are
     * present.
     ...
     *
     * <p>
     * Supports both Angular 2+ ({@code window.getAllAngularTestabilities()})
     ...
     * <li>React 16/17 — looks for {@code _reactRootContainer} on the root
     * element.</li>
     ...
     *
     * <p>
     * Common root ids checked: {@code #root}, {@code #__next} (Next.js),
     * {@code #app}.
     ...
     *
     * <p>
     * Enabled via {@code locators.selfHealing: true} in {@code testfly.yml}.
     * Each successful heal is recorded in {@link ExecutionMetrics} and surfaced in
     * the HTML report.
     *
     * @param locator the failing {@link By} locator
     * @return a visible {@link WebElement} found by a fallback strategy, or
     *         {@code null}
     */
    public static WebElement tryHeal(By locator) {
        if (!SelfHealingLocator.isEnabled())
            return null;
