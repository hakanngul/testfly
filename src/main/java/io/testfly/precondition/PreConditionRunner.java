 ...
 *
 * <p>
 * For each condition name declared on the test:
 * <ol>
 * <li>If a valid cached session exists for this thread → restore cookies +
 * localStorage.</li>
 * <li>Otherwise → find the {@link ConditionProvider} method, run it, cache the
 * session.</li>
 * </ol>
 *
 * <p>
 * On retry, the cache is invalidated so the provider runs fresh.
 */
public final class PreConditionRunner {
    ...
    /**
     * Evaluates all {@link PreCondition} annotations on the test method and
     * satisfies them.
     * No-op if the test has no {@code @PreCondition}.
     *
     * @param result the TestNG test result (used to read annotations and retry
     *               count)
     */
    public static void run(ITestResult result) {
        ...
        for (String conditionName : annotation.value()) {
            if (isRetry) {
                PreconditionSessionCache.invalidate(conditionName);
            }

            if (PreconditionSessionCache.isValid(conditionName)) {
     ...
     *
     * <p>
     * Looks for {@link PreCondition} on the test method, then on the declaring
     * class.
     * Runs or restores the session for each condition name.
     *
     * @param testMethod the test method about to execute
     * @param isRetry    true when this is a retry attempt — invalidates the cached
     *                   session
     *                   so the provider runs fresh rather than restoring a
     *                   potentially
    ...
    private static void restoreSession(String conditionName, WebDriver driver) {
        ...
        PreconditionSessionCache.restore(conditionName, driver);
    }
    ...
    private static void runProvider(String conditionName, WebDriver driver) {
        ...
        try {
            pm.invoke();
            PreconditionSessionCache.store(conditionName, driver);
            System.out.println("[PreCondition] Session cached for: " + conditionName);
        } catch (Exception e) {
            // InvocationTargetException wraps the real cause — unwrap it for a useful
            // message
            Throwable cause = (e.getCause() != null) ? e.getCause() : e;
            throw new RuntimeException(
                    "[PreCondition] Provider failed for condition '" + conditionName + "': " + cause.getMessage(),
                    cause);
        }
    }
}
