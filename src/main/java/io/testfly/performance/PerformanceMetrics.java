...
/**
 * Core Web Vitals and Navigation Timing metrics collected from the active
 * browser page.
 *
 * <p>
 * All time values are in <strong>milliseconds</strong> relative to the start of
 * the
 * page navigation. A value of {@code -1} means the metric was not available in
 * the current
 * browser (LCP and CLS are Chrome/Edge only; FCP requires Paint Timing API
 * support).
 *
...
public final class PerformanceMetrics {
    ...
    @JsonProperty
    private final double lcp; // ms — Largest Contentful Paint (Chrome/Edge only)
    @JsonProperty
    private final double fcp; // ms — First Contentful Paint
    @JsonProperty
    private final double fp; // ms — First Paint
    @JsonProperty
    private final double ttfb; // ms — Time To First Byte (responseStart)
    @JsonProperty
    private final double cls; // score — Cumulative Layout Shift (Chrome/Edge only)
    @JsonProperty
    ...
    /**
     * Largest Contentful Paint in ms. {@code -1} if not available (non-Chrome
     * browsers).
     */
    public double lcp() {
        return lcp;
    ...
    /**
     * Cumulative Layout Shift score (0–∞, lower is better). {@code -1} if not
     * available.
     */
    public double cls() {
        return cls;
    ...
    public double domLoad() {
        return domLoad;
    }

    /**
     * window.load event time in ms (full page including resources). {@code -1} if
     * not available.
     */
