...
public class NotificationAdapter implements ReportAdapter {
    ...
    /** Used by {@link io.testfly.lifecycle.FrameworkBootstrap} at init time. */
        ...
        try {
            ...
            if (slack != null && hasUrl(slack.getWebhookUrl())) {
                if (!slack.isNotifyOnFailureOnly() || failed > 0) {
                    ...
                    System.out.println("[TestFly] Slack notification sent (HTTP " + status + ")");
                }
            }
            ...
            if (teams != null && hasUrl(teams.getWebhookUrl())) {
                if (!teams.isNotifyOnFailureOnly() || failed > 0) {
                    ...
                    System.out.println("[TestFly] Teams notification sent (HTTP " + status + ")");
                }
            }

        } catch (Exception e) {
            System.err.println("[TestFly] Notification adapter failed: " + e.getMessage());
        }
    }
    ...
    String buildSlackPayload(JsonNode metrics) {
        ...
        header.putObject("text")
              .put("type", "plain_text")
              .put("text", icon + " TestFly — Suite Results");
    ...
    String buildTeamsPayload(JsonNode metrics) {
        ...
        payload.put("summary",    "TestFly Suite: " + status);

        ArrayNode sections = payload.putArray("sections");
        ObjectNode section = sections.addObject();
        section.put("activityTitle",    "**TestFly — Suite Results**");
