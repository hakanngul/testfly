...
public final class HookRegistry {
    ...
    public static synchronized void loadAll() {
        ...
        for (ExecutionHook hook : loader) {
            hooks.add(hook);
            System.out.println(
                    "[TestFly] ExecutionHook loaded: " + hook.getClass().getSimpleName());
        }
    }
    ...
    public static void onTestStart(String testId) {
        for (ExecutionHook h : hooks) {
            try {
                h.onTestStart(testId);
            } catch (Exception e) {
                warn("onTestStart", h, e);
            }
        }
    }
    ...
    public static void onTestFailure(String testId, Throwable cause) {
        for (ExecutionHook h : hooks) {
            try {
                h.onTestFailure(testId, cause);
            } catch (Exception e) {
                warn("onTestFailure", h, e);
            }
        }
    }
    ...
    private static void warn(String event, ExecutionHook hook, Exception e) {
        System.err.println(
                "[TestFly] Hook error [" + hook.getClass().getSimpleName()
                        + "#" + event + "]: " + e.getMessage());
    }
}
