...
public final class DbConnectionFactory {
    ...
    private static Connection createConnection(String name) {
        TestFlyConfig.Database config = resolveConfig(name);
        if (config == null || config.getUrl() == null || config.getUrl().isEmpty()) {
            throw new IllegalStateException(
                ...
                ". Add 'database.url' to testfly.yml."
            );
        }
    ...
    private static TestFlyConfig.Database resolveConfig(String name) {
        try {
            TestFlyConfig cfg = TestFlyContext.getConfig();
            if (cfg == null) return null;
            TestFlyConfig.Database db = cfg.getDatabase();
            if (db == null) return null;

            if (DEFAULT.equals(name)) return db;

            Map<String, TestFlyConfig.Database.DataSource> datasources = db.getDatasources();
            if (datasources == null) return null;
            TestFlyConfig.Database.DataSource ds = datasources.get(name);
            if (ds == null) return null;

            TestFlyConfig.Database wrapped = new TestFlyConfig.Database();
