...
/**
 * Generates a self-contained HTML trace file per failed (or optionally passing)
 * test.
 *
 * <p>
 ...
 * <li>Named step timeline — each step logged via {@code StepLogger} as a
 * clickable block</li>
 ...
 *
 * <p>
 * Output: {@code target/traces/{ClassName}/{testMethod}-trace.html}
 *
 * <p>
 * Activated automatically by {@code TestExecutionListener} when
 * {@code tracing.enabled: true} is set in {@code testfly.yml}.
 *
 * <pre>
 * # testfly.yml
 * tracing:
...
public final class TraceRecorder {
    ...
    public static String buildHtml(TestTiming timing, String testName, String finalScreenshotBase64) {
        ...
        sb.append("  <span class=\"step-count\">(").append(steps.size()).append(" step")
                .append(steps.size() != 1 ? "s" : "").append(")</span></div>\n");

        if (steps.isEmpty()) {
            sb.append(
                    "  <div class=\"no-steps\">No steps were logged. Use <code>StepLogger.step(\"name\", true)</code> to capture steps with screenshots.</div>\n");
        } else {
            ...
            for (int i = 0; i < steps.size(); i++) {
                ...
                sb.append("    <div class=\"step step-").append(stepStatus).append("\"")
                        .append(clickAttr)
                        .append(" title=\"").append(escHtml(s.getName())).append(" (+").append(s.getOffsetMs())
                        .append("ms)\">\n");
