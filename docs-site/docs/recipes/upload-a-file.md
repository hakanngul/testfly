---
description: "Upload a file in Selenium without OS dialogs: TestFly's upload(By, path) sets the file input directly, with paths resolved from the classpath or an absolute location."
id: upload-a-file
title: Upload a file
...
- The path may be **classpath-relative** (e.g. `testfiles/resume.pdf`) or **absolute**. TestFly resolves it and fails fast with a clear error if the file doesn't exist.
