---
description: "Automatic Web UI video recording on test failure, modeled after Playwright retain-on-failure, embedded directly in TestFly HTML and Allure reports."
id: video-recording
...
When enabled, TestFly records the live browser interaction during test execution. If the test passes, the buffered frames are discarded immediately. If the test fails, TestFly stitches the frames into an animated recording file (`.gif`) and embeds it directly into the interactive HTML report, Allure results, and test traces.
