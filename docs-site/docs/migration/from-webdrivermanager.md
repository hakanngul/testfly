---
description: "Migrate off WebDriverManager: modern Selenium bundles Selenium Manager, so TestFly resolves driver binaries with zero WebDriverManager code — delete the setup calls and the dependency."
...
If your project calls `WebDriverManager.chromedriver().setup()` (or Boni García's `io.github.bonigarcia:webdrivermanager` in general) to download browser drivers, you can **delete all of it**. Modern Selenium resolves driver binaries itself, and TestFly uses that mechanism out of the box.
...
TestFly is built on modern Selenium, so this is already active. You don't call it, configure it, or depend on it directly.
...
**After** — nothing. There is no `setup()` call and no WebDriverManager dependency. With TestFly you don't even create the driver — extend [`BaseTest`](/docs/guides/base-test) and the framework does it:
